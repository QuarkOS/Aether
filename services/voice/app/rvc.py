...
_DEFAULT_F0 = "rmvpe"
# quality: 0.3 keeps Alya timbre; fast: 0 is snappier (~30–40% on short clips) but thinner.
_QUALITY_INDEX_RATE = 0.3
...
def load_model(name: str) -> bool:
    ...
    with _lock:
        try:
            ...
            _apply_params(engine, 0, "quality")
            _loaded_model = name
...
def convert(wav_bytes: bytes, model: str, pitch: int = 0, mode: str = "quality") -> bytes:
    """Convert input WAV bytes to the target voice. Falls back to input on error."""
    quality = "fast" if mode == "fast" else "quality"
    with _lock:
        try:
            engine = _get_engine()
